...
const ADMIN_MSG = {
  waiting_dp:             { text: '⏳ Silakan bayar DP 30% untuk memulai proses pernikahan Anda.', color: 'bg-amber-50 border-amber-200 text-amber-700' },
  payment_failed:         { text: '❌ Pembayaran gagal. Silakan coba lagi.', color: 'bg-red-50 border-red-200 text-red-600' },
  waiting_payment:        { text: '⏳ Silakan bayar DP 30% untuk memulai proses pernikahan Anda.', color: 'bg-amber-50 border-amber-200 text-amber-700' },
  waiting_vendor:         { text: '✅ DP diterima! Admin sedang memilih vendor terbaik untuk Anda.', color: 'bg-blue-50 border-blue-200 text-blue-700' },
  ...
  preparation:            { text: '🎪 Persiapan acara berjalan. Silakan lunasi sisa pembayaran (70%).', color: 'bg-indigo-50 border-indigo-200 text-indigo-700' },
...
export default function CustomerMyBookings() {
  ...
  async function handlePayFull(booking) {
    try {
      ...
      const token = sessionStorage.getItem('amaranta_token') || localStorage.getItem('amaranta_token')
      ...
      if (!res.ok) { alert(data.message || 'Belum bisa lunasi sekarang.'); return }
      // Buka Midtrans Snap
    ...
    } catch (err) { alert('Tidak bisa terhubung ke server.') }
  }
              ...
              <div className="p-5">
                ...
                {ADMIN_MSG[booking.admin_status] && (
                  <div className={['mb-4 px-3 py-2 border text-xs font-[var(--font-sans)]',
                    (ADMIN_MSG[booking.admin_status]?.color || 'bg-[var(--color-cream)] border-[var(--color-cream-border)] text-[var(--color-dark-muted)]')].join(' ')}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {(booking.admin_status === 'waiting_dp' || booking.admin_status === 'waiting_payment') && (
                    <a href={'/pesan/' + booking.package?.tier_id}
                      ...
                      💳 Bayar DP 30% — {booking.total_price ? formatRupiah(Math.round(booking.total_price * 0.3)) : ''}
                    </a>
                  )}

                  {/* Lunasi — hanya saat preparation + DP sudah bayar */}
                  {booking.admin_status === 'preparation' && booking.phase === 'dp_paid' && (
                    <button
                      ...
                      💰 Lunasi Sisa 70% — {booking.total_price ? formatRupiah(Math.round(booking.total_price * 0.7)) : ''}
                    </button>
                  )}

                  {/* Invoice — setelah bayar DP */}
                  {['dp_paid','paid','in_event','rated'].includes(booking.phase) && (
                    <Link to={'/pelanggan/invoice/' + booking.id + '?type=' + (['paid','in_event','rated'].includes(booking.phase) ? 'full' : 'dp')} target="_blank"
                      ...
                      {['paid','in_event','rated'].includes(booking.phase) ? 'Invoice Lunas' : 'Invoice DP'}
                    </Link>
                  )}
